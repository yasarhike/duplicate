...
public final class StoryServiceDAOImpl implements StoryServiceDAO {
    ...
    private Story setStoryUnique(final Story story, final ResultSet resultSet) {
        try {
            if (resultSet.next()) {
                ...
                story.setText(resultSet.getString(3));
                story.setPrivate(resultSet.getBoolean(4));
                story.setMusic(resultSet.getString(5));
                story.setTimestamp(resultSet.getTimestamp(6));
                return story;
            }
        } catch (final SQLException exception) {
        }
    ...
    private String getStoryDisplayQuery() {
        return String.join(" ", "select story.id, story.user_id"
                , " ,  story.caption, story.is_private, story.music, story.created_at from story"
                , " where story.user_id = ? "
                , " order by story.id;");
    }
    ...
    private String getUniqueStory() {
        return String.join(" ", "select id, user_id"
                , ",  caption, is_private, music, created_at ,"
                , " from story where id = ?;");
    }
}
